...
export const DialogFrame = (({open, handleClose, title, button, children})=>{
    return <>
        ...
        >
            ...
            <DialogActions>
            <Button autoFocus variant='contained' disabled={button?.disabled} sx={{ width:'90%', borderRadius: '220px', font:'400 normal 14px Arial', height: '45px', margin: '32px 0 32px 0', backgroundColor: button?.disable?'#333':'#d9001b' }} onClick={button?.action?button?.action:handleClose}>
                {button?.title}
            </Button>
            </DialogActions>
        </BootstrapDialog>
